...
- **页面风格**:所有页面引用 `../code-style.css` 与 `../code-copy.js`,代码框用 `class="code-box"` + 复制按钮,说明用 `class="info-box"`。新增页面务必复用这两个文件,保持风格统一。
- **更新日志**:每次发布在 `data/更新日志/更新日志.htm` 中追加,并把 `data/检查更新/检查更新.htm` 里的 `CONFIG.current` 同步为最新版本号。
- 编译产物(UltraLib.chm)复制到仓库 `dist/` 随发布提供。

## 与仓库其它文档的关系

- `docs/`(Markdown)→ 在线 Wiki / CHM 内容源
- `tools/chm/`(build-chm.js)→ 旧的自动 Markdown→CHM 管线(可保留备用)
- `tools/chm-win/`(本工程)→ **WinCHM 手工整理版**(当前主用,风格更佳)
